...
else if($choosefile == "Pdf")
{
	...
	if(isset($_FILES['template1addpdf'.$i]['name']))
	{
		...
		$thumbDirectory = "template1pdffile/";
	
        $thumb = basename($name, ".pdf");
        $newname=preg_replace("/[^A-Za-z0-9_-]/", "", $name).".pdf";


		   if(move_uploaded_file($_FILES['template1addpdf'.$i]['tmp_name'], $path.$name)) 

		   { 
		   	$pdfWithPath = $path.$newname;
		   	 
		   	$thumb = $thumb.".jpg";

		   	$pdfimagedir= $thumbDirectory.$thumb;


		   	exec("convert \"{$pdfWithPath}[0]\" -scale x400 -quality 75  -flatten $thumbDirectory$thumb");

			$template1topimagequery=mysql_query("INSERT INTO `template1_carousel`
			(template1_rightimages,temp1imgcaption, template_content_id,contentchecked,pdfimages)
			 VALUES('$paname','$template1imgname', '$temp_content_id','$choosefile','$pdfimagedir')");
...
$tempeveninsert = mysql_query("INSERT INTO `template3event` (tl3_id, eventname, description, time, date, place, created_on) VALUES('$getInserId', '$event3name', '$event3desc', '$event3time', '$event3date', '$event3place', '$createdOn' )");
		...
		if($Screen != 'screen7')

			{
				...
				echo '<meta http-equiv="refresh" content="0;SubScreen1.php?screenid='.$PassId.'&screen='.$Screen.'" />';
           ...
           <div class="section2">
                ...
                <div id="template1rightfiles">              
                    ...
                    <div class="subSec1" id="appendClass1">  
                     ...
                     <div style="clear:both"></div>
                     <span style='color:red;font-size:14px'>You can add minimum 6 files and maximum 10</span>
