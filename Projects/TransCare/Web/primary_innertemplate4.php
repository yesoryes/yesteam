    ...
    <div class="ip_container">
      ...
      <div class="acc_executive_right">
       ...
       //echo "SELECT * FROM ".$gettn." WHERE template_content_id = '$gettid'";
        ...
        $fetchlink = mysql_fetch_array($link4temp);
        $link4id = $fetchlink['id'];

       $geteven = mysql_query("SELECT * FROM `primary_template4_event` WHERE tl4_id = '$link4id'");
       $getcount = mysql_num_rows($geteven);
       
       
        $i=1;

        while($fetlink = mysql_fetch_array($geteven))   
        {           

          if($i == 1) 
            { echo '<div class="item">'; }else{ echo ''; }

        ?>        
            <span class="anchor_acc_exective myModalFunction" data-fun="<?php echo $i; ?>"><a>
              <div class="acc_executive">
                <div class="acc_content">
                  <h3><?php echo $fetlink['headername'] ?></h3>
                  <p><?php echo ucfirst(substr($fetlink['description'], 0, 100)); ?></p>
                </div>
                <ul class="acc_details">
                  <li>Division</li>
                  <li><?php echo $fetlink['division'] ?></li>
                  <li>Type</li>
                  <li><?php echo $fetlink['type'] ?></li>
                  <li>Available</li>
                  <li><?php echo $fetlink['available'] ?></li>
                  <li>Place</li>
                  <li><?php echo $fetlink['place'] ?></li>
                </ul>
          ...
          <?php
          $getpopupdata = mysql_query("SELECT * FROM primary_template4_event WHERE tl4_id = '$link4id'");
          $j = 1;
        ...
        <div id="myModal<?php echo  $j; ?>" class="modal_box popbox">
          <div class="popup_container  account_exe ">
            ...
            <div class="account_exe contentHolder">
            	<div class="scr_calendar boxscroll" id="">
            <!-- <div id="scr-calendar-pop" style="height:400px; width:400px; overflow:hidden; background-color:#FF9; position:absolute">
            	<div>-->
  ...
  <footer class="foo_ip">
      <div class="container">
        ...
        $sel1 = mysql_query("SELECT * FROM primary_template_content WHERE parentid = '$cur'");
            ...
            <a href="primary_innertemplate<?=$row1['template_id']?>.php?dbname=<?php echo $row1['tablename']; ?>&tcid=<?php echo $row1['id']; ?>&cur=<?php echo $cur; ?>" class="foo_btn"><?php echo $row1['linkname']; ?></a>  
            </div>
            <div class="btn_right"></div>
            </div> 
            <?php
